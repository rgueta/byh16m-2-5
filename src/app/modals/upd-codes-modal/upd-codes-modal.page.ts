  ...
  async onSubmitTemplate(){
    ...
    try{
      ...
      this.api.postData('api/codes/' + this.userId,{'code':this.code,'sim':this.visitorSim,
       ...
       'source': {'user' : this.userId,'platform' : this.StrPlatform, 'id' : userSim.value}}).then(resp => {
       ...
       this.sendSMS(coreSim['value'], pckgToCore);

       console.log('pckg to core --> ', pckgToCore);

 
       // send code to visitor
       this.sendSMS(this.visitorSim,'codigo ' + coreName['value'] + ': ' + this.code + 
       '  Expira en ' + expire + ' Hrs.' ) //+ Utils.convDate(this.expiry))
